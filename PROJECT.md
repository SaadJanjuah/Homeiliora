...
- [x] **Poster layout, left-aligned, with a horizontal gradient scrim** — built to a supplied reference. Type sits left over the dense end of the gradient; the gradient clears toward the right so the room stays sharp. A flat wash strong enough to carry light type was tried first and flattened the photographs completely ("completely blurred"), which is what the gradient exists to avoid.
