...
- [x] Light/dark theming with a header toggle beside the search icon. **The visitor's device decides the starting theme** (`prefers-color-scheme`): a device set to dark gets the dark site, anything else gets light. Clicking the toggle stores an explicit choice in localStorage (`homeiliora_theme_v1`) that overrides the device in both directions and syncs across tabs.
      The device trigger is a CSS media query rather than something JavaScript derives, so it still works with scripting unavailable; the cost is that the variable mappings in `dark.css` appear twice, though the hex values are declared once. The toggle's own "which theme is showing" check has to consult the device in exactly the same way — otherwise the button labels and paints itself for a theme the reader isn't looking at.
      **Known gap:** the toggle is two-state. Once a reader clicks it, the choice is stored forever and the site stops following their device; there is no "back to automatic". Clearing the site's storage is currently the only reset.
      (Briefly changed to always-light on 2026-08-15, then reverted the same day to device-following on request.)
...
- [x] **Full-bleed poster band, left-aligned, with a horizontal gradient scrim** — built to a supplied reference. The photograph runs edge to edge (`alignfull`); the type sits left over the dense end of the gradient, aligned to the site's content column, and the gradient clears toward the right so the room stays sharp. A flat wash strong enough to carry light type was tried first and flattened the photographs completely ("completely blurred"), which is what the gradient exists to avoid.
- [x] Band is ~4:1 (29.5rem min-height) and the headline is ~52px at desktop, matching the reference's proportions — the first attempt at 64px filled the band instead of sitting in it.
- [x] **The sticky ad rails hide while the hero is on screen** (`assets/js/ad-rails.js`, IntersectionObserver) and fade back in once it scrolls away. Going full-bleed put the fixed rails directly over the photograph, which looks broken and is the sort of placement ad networks object to. Pages without a hero are untouched; without IntersectionObserver the rails behave as before.
- [x] The hero is a dark photographic band in **both** themes, with light type. Normal ink-on-plaster type cannot work here: the eyebrow is sage and the dek is muted, and both need a background nearly as bright as plaster to clear 4.5:1. Precedent: the footer is already a dark band in both themes.
- [x] Two-tone headline (first word terracotta), a dek, and an **Explore Ideas** button that anchors to `#ideas` on the moodboard grid. Verified the scroll lands the grid clear of the sticky header.
- [x] **Headline is Space Grotesk, not Playfair** — a deliberate narrow exception to the type split in §2.2. The hero is a poster rather than an article opening, and the reference leads with a geometric sans. Post titles and content headings keep Playfair.
- [x] Scrim was set by **sampling the rendered pixels** behind each line of type — hero captured with the text hidden, at three points in the fade, in both themes, scoring the worst pixel rather than the average. Worst case across all 24 samples is **5.06:1**.
